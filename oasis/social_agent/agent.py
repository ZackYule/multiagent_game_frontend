...
user2 = User(name="Shiguan", avatar="avatars/截屏2024-12-22 01.19.53.png")
user3 = User(name="CoCo", avatar="avatars/截屏2024-12-22 01.20.02.png")
user4 = User(name="Panguan", avatar="avatars/截屏2024-12-22 01.20.10.png")
user5 = User(name="Lingpo", avatar="avatars/截屏2024-12-22 01.20.19.png")
user6 = User(name="Weizhi", avatar="avatars/截屏2024-12-22 01.20.24.png")
user7 = User(name="Jiaoshou", avatar="avatars/image.png")
...
class SocialAgent:
        ...
        if self.is_openai_model:
            ...
            for tool_call in response.choices[0].message.tool_calls:
                ...
                if action_name == "create_post":
                    new_message = Message(
                        user=users[self.agent_id], content=args["content"].strip()
                    )
            ...
            while retry > 0:
                ...
                mes_id = await self.infe_channel.write_to_receive_queue(openai_messages)
                mes_id, content = await self.infe_channel.read_from_send_queue(mes_id)
                ...
                try:
                    ...
                    for function in functions:
                        ...
                        if name != "do_nothing":
                            ...
                            arguments = {}
                        exec_functions.append({"name": name, "arguments": arguments})
    ...
    async def perform_test(self):
        ...
        message_id = await self.infe_channel.write_to_receive_queue(openai_messages)
        message_id, content = await self.infe_channel.read_from_send_queue(message_id)
        agent_log.info(f"Agent {self.agent_id} receive response: {content}")
            ...
            while True:
                try:
                    if st.session_state.task_queue:
                        value = st.session_state.task_queue.pop(0)
